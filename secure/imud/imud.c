...
inherit __DIR__ "imud/daemon_data";
inherit __DIR__ "imud/reconnect";
...
#define SOCKET __DIR__ "socket.c"
...
private nomask void reconnect()
{
  ...
  err = catch(router_socket = evaluate((: clone_object, SOCKET, SKT_STYLE_CONNECT_M,
      router_list[0][1],
  (: handle_router_read :),
  (: handle_router_close :) :)));
